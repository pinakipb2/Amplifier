...
const AddPodcasts = () => {
  ...
  const podcastSchema = z.any().refine((files) => files?.[0]?.size <= MAX_FILE_SIZE, `Max image size is 50MB.`);
  ...
  const onSubmit: SubmitHandler<podcastFormSchemaType> = async (data) => {
    try {
      ...
      fileReader.onload = async () => {
        ...
        console.log(srcData);
        const res = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/add-podcast`, {
          name: data.name,
        ...
        //@ts-ignore
        setAllPodcast((oldPodcast) => [...oldPodcast, res.data.result]);
      };
